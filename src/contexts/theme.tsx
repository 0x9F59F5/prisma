...
import { localStorageSync } from '@/utils/storage';
...
export const ThemeProvider = ({ children }: React.PropsWithChildren) => {
    ...
    const handleThemeChange = useCallback((value: ThemeValue, persist = true) => {
        ...
        if (persist) {
            localStorageSync.set(LOCAL_STORAGE_THEME_KEY, value);
        }
    }, []);
    ...
    const handleColorSchemeChange = useCallback(
        (event: MediaQueryListEvent) => {
            if (localStorageSync.get<ThemeValue>(LOCAL_STORAGE_THEME_KEY)) {
                return;
            }
...
function getPreferableTheme(): ThemeValue {
    const storedThemeValue = localStorageSync.get<ThemeValue>(LOCAL_STORAGE_THEME_KEY);
