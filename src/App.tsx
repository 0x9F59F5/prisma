...
// import { migrateToSignalDB } from '@/utils/migrateStorage';
...
const App = () => {
  ...
  // localStorage에서 SignalDB로 데이터 마이그레이션 (최초 1회)
  // useEffect(() => {
  //   const migrate = async () => {
  //     await migrateToSignalDB();
  //   };
  //   migrate();
  // }, []);
  ...
  useEffect(() => {
    ...
    const loadCanvasState = async () => {
      const storedCanvasState = await storage.get<AppState>(LOCAL_STORAGE_KEY);
  ...
  useEffect(() => {
    const loadLibrary = async () => {
      const storedLibrary = await storage.get<Library>(LOCAL_STORAGE_LIBRARY_KEY);
